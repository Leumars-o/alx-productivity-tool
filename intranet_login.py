...
def fetch_projects(cred):
    ...
    with requests.Session() as session:
        ...
        payload = {
            ...
            "user[email]": f"{cred['email']}",
            "user[password]": f"{cred['password']}",
            "user[remember_me]": 0,
            "commit": "Log+in"
        }
        ...
        if current_url == "https://intranet.alxswe.com/dashboard" \
            or current_url == "https://intranet.alxswe.com/":
            ...
            print(current_url)
            print("Checking for evaluation quiz...")
        ...
        target_panel = soup.find_all('div', class_='panel panel-default')
        for panel in target_panel:
...
def main(credentials):
    projects = fetch_projects(credentials)
    return projects
